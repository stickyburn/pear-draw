...
export async function showStudioConnectionModal(onStartHost, onJoinHost, onDismiss) {
  const html = `
  ...
  await modalFlow({
    inputConfig: {
      ...
      preConfirm: () => {
        ...
        if (!profile) { swal.showValidationMessage("Please enter your name"); return false; }
        return { profile };
      },
    },
    ...
    onSuccess: async (invite) => {
      ...
      await swal.fire({
        ...
        customClass: { popup: 'studio-modal', confirmButton: 'studio-btn-primary' },
        buttonsStyling: false,
      });
    },
  });
  ...
  await modalFlow({
    inputConfig: {
      ...
      preConfirm: () => {
        ...
        if (!profile || !invite) { swal.showValidationMessage("Missing information"); return false; }
...
export function StudioFab(props) {
  ...
  return (
    <button
      onClick={() => showStudioConnectionModal(props.onStartHost, props.onJoinHost, props.onDismiss)}
